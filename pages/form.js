...
import CFCProgressDots from '../components/ui/progressdots';
// import { questions } from '../components/ui/questions/testquestions'
import CFCParseQuestion from '../components/ui/questions/questionbuilder'
import CFCNextPrevButtons from '../components/ui/nextPrevArrows';
import { queryQuestions, sendAnswersArray } from "../helpers/db";
import CFCFilledButton from '../components/ui/filledbutton';
import CFCSubmitAnswers from '../components/ui/submitanswers';
...
export default function Home( theme ) {
      ...
      next({user,userInfo}){
        if(user){
          if (!questionsBuilt) {
            queryQuestions().then(
              (queriedQuestions) => {
                // console.log('queriedQuestions', queriedQuestions)
                if (queriedQuestions && queriedQuestions.length > 0) {
                  setUserAnswers(userInfo.answers)
                  setQuestions(queriedQuestions)
                  setNumQuestions(queriedQuestions.length)
  ...
  const handleNext = () => {
    ...
    if (activeStep >= numQuestions) {
      // Do nothing
      // Next button should be deactivated
    }
    else if (activeStep == numQuestions - 1) {
      // Update the text for the submit component that says what questions you need to answer still or if you are ready to submit
      ...
      // Long term maybe set the corresponding question dots you haven't answered to a light red for clairity
    }
    else {
      sendAnswers();
      setActiveStep((prevActiveStep) => prevActiveStep + 1);
                        ...
                        {/* Grid to space the progress bar */}
                          <Grid item>
                            ...
                            <Grid item style={{height: '50vh'}}>
                              ...
                              {showEnd ? (
                                <CFCSubmitAnswers key={submitMessage} submitMessage={submitMessage} numQuestions={numQuestions} numAnswers={checkCompleted} onClick={() => {
