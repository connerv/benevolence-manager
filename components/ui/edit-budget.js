...
import { updateUserBudget } from "../../helpers/db";
...
export default function EditBudget( {user, uid, resetData} ) {
  ...
  React.useEffect(() => {
    ...
    if(user.income && user.income != ""){
        let income = JSON.parse(user.income)
        console.log(income)
        setData1(income)
    }
    if(user.expenses && user.expenses != ""){
        let expenses = JSON.parse(user.expenses)
  ...
  const submit = async () =>{
    ...
    await updateUserBudget(income, expenses, uid)
    resetData()
    handleClickOpen()
